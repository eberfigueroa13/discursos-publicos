...
function renderDash(){
  ...
  var oe=D.cargaMensual.length;
  var bloq=D.cargaMensual.filter(function(cm){var u=ultimaVez(cm.numDiscurso);return u&&diasDesde(u,refMes())<db;}).length;
  // Stats del mes activo
  var planMes=D.planificacion.filter(function(p){var ym=c.anio+'-'+String(c.mes).padStart(2,'0');return p.fecha&&p.fecha.slice(0,7)===ym;});
  ...
  var stats=[
    {l:'Mes coordinado',v:nMes(c.mes)+' '+c.anio},
    {l:'Congr. externa mes',v:c.congregacionExternaMes||'---'},
    {l:'Fechas del mes',v:extMes.length,s:confirmados+' confirmadas / '+sinAsignar+' sin asignar'},
    {l:'Dia de reunion',v:nomDia(mc.dia)||'---'},
    {l:'Discursos catalogo',v:D.discursos.filter(function(d){return d.estado==='Activo';}).length,s:'activos'},
    {l:'Hermanos activos',v:D.locales.filter(function(h){return h.estado==='Activo';}).length,s:'locales'},
    {l:'Congregaciones ext.',v:D.congregaciones.filter(function(c){return c.estado==='Activa';}).length,s:'activas'},
    {l:'Arreglos año',v:(D.arreglos||[]).filter(function(a){return parseInt(a.anio)===parseInt(c.anio);}).length,s:'registrados para '+c.anio},
    {l:'Historial interno',v:D.historial.length,s:'discursos en nuestra congregacion'},
    {l:'Salidas realizadas',v:D.salidasRealizadas.length,s:'a otras congregaciones'},
    {l:'Carga del mes',v:oe,s:bloq+' bloqueadas / '+(oe-bloq)+' disponibles'}
  ];
  document.getElementById('dstats').innerHTML=stats.map(function(s){
    var fs=typeof s.v==='string'&&s.v.length>8?'15px':'28px';
    return '<div class="sc"><div class="sl">'+s.l+'</div><div class="sv" style="font-size:'+fs+'">'+esc(s.v)+'</div>'+(s.s?'<div class="ss">'+s.s+'</div>':'')+'</div>';
  }).join('');
  document.getElementById('dinfo').innerHTML='<div class="ctit">Mi congregacion</div>'
    +'<div style="display:flex;flex-wrap:wrap;gap:16px;font-size:13px;color:var(--tx2)">'
    +(mc.nombre?'<span>'+esc(mc.nombre)+'</span>':'<span style="color:var(--tx3)">No configurada</span>')
    +(mc.direccion?'<span>'+esc(mc.direccion)+'</span>':'')
    +(mc.dia?'<span>Discurso: '+nomDia(mc.dia)+' '+esc(mc.horario)+'</span>':'')
    +(mc.coordNombre?'<span>Coord: '+esc(mc.coordNombre)+'</span>':'')
    +'</div>';
}
...
function estilosCartaPlanificacionPDF(){
  return [
    '@page{size:letter;margin:0.62in 0.68in 0.62in 0.68in;}',
    'body{font-family:Arial,Helvetica,sans-serif;font-size:10.5pt;color:#1f2937;margin:0;line-height:1.38;background:#fff;}',
    'p{margin:0 0 10px 0;line-height:1.55;text-align:justify;}',
    'table{width:100%;border-collapse:collapse;table-layout:fixed;}',
    'td,th{font-size:10pt;vertical-align:top;word-wrap:break-word;overflow-wrap:break-word;}',
    '.letter-hero{margin:0 0 18px 0;page-break-inside:avoid;}',
    '.letter-hero td{background:#EAF2FB;border:1px solid #B9CCE2;padding:18px 22px;text-align:center;}',
    '.hero-title{font-size:18px;font-weight:bold;color:#1F4E79;letter-spacing:.06em;text-transform:uppercase;margin:0 0 4px 0;}',
    '.hero-sub{font-size:13px;color:#44546A;font-weight:bold;margin:0;}',
    '.hero-month{font-size:12px;color:#5B6778;margin-top:6px;}',
    '.to-line{font-size:11px;margin:0 0 14px 0;}',
    '.section-title{font-size:11px;text-transform:uppercase;font-weight:bold;color:#1F4E79;margin:0 0 8px 0;letter-spacing:.04em;}',
    '.contact-box{border:1px solid #C9D8EA;background:#F8FBFF;margin:15px 0 18px 0;page-break-inside:avoid;}',
    '.contact-box td{padding:7px 9px;border-bottom:1px solid #E2E8F0;}',
    '.contact-label{width:34%;font-weight:bold;color:#344054;background:#F1F6FC;}',
    '.info-box{border-left:5px solid #D6A64F;background:#FFFAF0;border-top:1px solid #F2DCA8;border-right:1px solid #F2DCA8;border-bottom:1px solid #F2DCA8;margin:15px 0 16px 0;padding:13px 15px;page-break-inside:avoid;}',
    '.info-title{font-size:11px;font-weight:bold;color:#8A5A00;text-transform:uppercase;margin:0 0 8px 0;}',
    '.firma-wrap{margin-top:36px;text-align:center;page-break-inside:avoid;}',
...
async function dbLoadAll(cb){
  ...
  try {
    ...
    D.congregaciones = (res[5].data||[]).map(function(c){ return {id:c.id,nombre:c.nombre,direccion:c.direccion||'',dia:c.dia||'0',horario:c.horario||'',coordNombre:c.coord_nombre||'',coordTel:c.coord_tel||'',coordEmail:c.coord_email||'',obs:c.obs||''}; });
